    ...
    vector<Document> FindTopDocuments(const string& raw_query,
        ...
        sort(matched_documents.begin(), matched_documents.end(),
             [](const Document& lhs, const Document& rhs) {
                 if (abs(lhs.relevance - rhs.relevance) < m6) {
...
void TestAddDocument() { //тест на добавление документа
    const int doc_id = 42;
    const string content = "cat in the city"s;
    const vector<int> ratings = {1, 2, 3};
...
void TestStatus() {
    const int doc_id = 41;
    const string content = "cat in the city dog"s;
    const vector<int> ratings = {1, 2, 3};
    ...
    const string content3 = "dog"s;
    const vector<int> ratings3 = {10, 4, 2};
    
    const int doc_id4 = 88;
    const string content4 = "dog universe"s;
    const vector<int> ratings4 = {10, 4, 2};
    ...
    found_docs = server.FindTopDocuments("dog"s, DocumentStatus::BANNED);
    ASSERT_EQUAL(found_docs.size(), 1);
    
    found_docs = server.FindTopDocuments("dog"s, DocumentStatus::IRRELEVANT);
    ASSERT_EQUAL(found_docs.size(), 1);
    found_docs = server.FindTopDocuments("dog"s, DocumentStatus::REMOVED);
    ASSERT_EQUAL(found_docs.size(), 1);
